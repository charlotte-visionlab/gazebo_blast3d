...
void GazeboBlast3DCameraPlugin::Load(sensors::SensorPtr _sensor, sdf::ElementPtr _sdf) {
  ...
  getSdfParam<std::string>(_sdf, "blast3dRGBImageTopic",    blast3d_rgb_image_topic_,    blast3d_rgb_image_topic_);
  getSdfParam<std::string>(_sdf, "blast3dEventImageTopic",  blast3d_event_image_topic_,  blast3d_event_image_topic_);
  getSdfParam<std::string>(_sdf, "blast3dEventTopic",       blast3d_event_topic_,        blast3d_event_topic_);
  getSdfParam<std::string>(_sdf, "blast3dVideoDataFolder",  blast3d_video_datafolder_,   blast3d_video_datafolder_);
  getSdfParam<std::string>(_sdf, "cameraMode",              camera_mode_,                camera_mode_);
  getSdfParam<std::string>(_sdf, "blast3dServerRegisterLinkTopic", blast3d_server_reglink_topic_, blast3d_server_reglink_topic_);
  getSdfParam<std::string>(_sdf, "blast3dServerLinkTopic",  blast3d_server_link_topic_,  blast3d_server_link_topic_);
  ...
  meas_loc_csv_.open("/home.md2/sparab2/wind/uncc_wind_control/ros_image/ros_ws/src/gazebo_blast3d/datasets/measured_location_log.csv",
                     std::ios::out);
  if (meas_loc_csv_.is_open()) {
    meas_loc_csv_ << "event_id,sim_time,meas_x,meas_y,meas_z\n";
  } else {
    gzwarn << "[camera] Unable to open measured_location_log.csv for writing.\n";
  }
  if (meas_loc_csv_.is_open()) meas_loc_csv_.close();
}
...
void GazeboBlast3DCameraPlugin::OnNewFrameCamera(const unsigned char * _image) {
  ...
  // ---- check blasts & emit one-shot event_cam BEFORE rate gate ----
  for (auto it = blastMsgList.begin(); it != blastMsgList.end(); ++it) {
    if (it->time() < current_time_double) {
      current_event_id_ = event_id_map_[it->time()];
      if (current_event_id_ >= 0 && !logged_event_cam_eids_.count(current_event_id_)) {
        const double sim_t = world_->SimTime().Double();
        blast3d_sync::publishSyncLog(sync_pub_, "event_cam",
                                     current_event_id_, sim_t, vehicle_id_,
                                     last_standoff_dist_);
        logged_event_cam_eids_.insert(current_event_id_);
        gzdbg << "[event_cam] EID=" << current_event_id_
              << " t=" << sim_t
              << " standoff=" << last_standoff_dist_
              << " vehicle=" << vehicle_id_ << std::endl;
      }
      explosion_triggered_ = true;
      blastMsgList.erase(it);
      break;
    }
  }
  // ------------------------------------------------------------------
...
void GazeboBlast3DCameraPlugin::blendEventOutput(double roll) {
  ...
  // blast3d_sync::publishSyncLog(sync_pub_, "event_cam", current_event_id_, sim_t, vehicle_id_, last_standoff_dist_);
...
void GazeboBlast3DCameraPlugin::Blast3DCallback(Blast3dMsgPtr & blast3d_msg) {
  ...
  // If x,y,z are BLAST→VEHICLE relative in your system:
  ignition::math::Vector3d rel(blast3d_msg->x(),
                               blast3d_msg->y(),
                               blast3d_msg->z());
  last_standoff_dist_ = rel.Length();
}
